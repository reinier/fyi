---
title: Iedereen heeft een keymap, jij ook!
---

![Overview van mijn keymap](/images/blog/keymap/keymap-overview.jpg)

Hallo ouwe toetsenbordtijger! Ik ben hier om je te vertellen dat je een keymap hebt, ook al heb je er misschien nog nooit van gehoord. En een goede keymap kan het verschil betekenen tussen je gewrichten slopen door alle vingeryoga die je op een werkdag doet en gezonde handen en polsen als je straks 60 bent.
...
De meeste toetsenborden hebben een keymap waar je niks aan kan veranderen. Het zit voorgeprogrammeerd op je toetsenbord en de meeste mensen realiseren zich niet eens dat het aanpassen van een keymap überhaupt een ding is.

Het wordt een ander verhaal als je een eigen toetsenbord gaat maken of een toetsenbord koopt die de mogelijkheid heeft om je keymap aan te passen (zoals de meeste nieuwe [Keychron toetsenborden](https://www.keychron.com/products) bijvoorbeeld). Ineens merk je dat het verdraaid handig is als je je eigen voorkeuren helemaal kan inregelen.

Een paar handigheidjes als je je eigen keymap kan samenstellen:
...
En andere veelgebruikte toepassing hierbij is de `capslock` die iedereen altijd in de weg zit. Maak er een HYPER-key van bij hold (`ctrl-alt-shift-cmd` tegelijk) en bij een tap de `esc` toets. Nu kun je zelf sneltoetsen op je computer inregelen die gebruik maken van deze HYPER-toets. Ik ga hier later weleens wat meer over vertellen, want de HYPER-toets is een eigen post waard.
...
- Ik een toets waarmee ik de huidige selectie ‘opzuig’ en met een tap weer uitspuug. Alsof het een stempel is. Hold is dus `cmd-c` en tap is `cmd-v`.
